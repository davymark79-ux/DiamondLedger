...
//
// Calibrated against a real season-1 payroll run across all 50 teams (a
// throwaway smoke test, same "smoke-test before wiring into data/season.js"
// precedent as the Ledger Cup arc) — a real, non-obvious finding drove the
// final numbers: the distribution is genuinely BIMODAL, not a single bell
// curve, because MLB1/MLB2's pre-existing, deliberate roster-quality gap
// (validate:roster's own "MLB1/MLB2 quality gap" check) translates directly
// into a payroll gap once salary is purely quality-driven — MLB2 clubs
// clustered ~$50-70M, MLB1 clubs ~$120-180M, with almost no overlap. These
// two constants are set just outside each cluster's typical range (floor
// below most of the MLB2 cluster, threshold above most of the MLB1
// cluster) so the mechanic hits a real minority of teams (~5-10%) on each
// end rather than an entire tier permanently, or nobody ever.
export const SALARY_FLOOR = 55_000_000;
export const LUXURY_TAX_THRESHOLD = 165_000_000;
